    ...
    <View style={{ flex: 1, backgroundColor: settingsBg }}>
      ...
      >
        ...
        <Section
          ...
          body={"We use the following services to operate the app:\n\n• Supabase — database and authentication\n• Apple Sign-In — optional sign-in method\n• Google Sign-In — optional sign-in method\n• Anthropic — powers the optional AI suggestions (see below)\n\nThese services have their own privacy policies. We send them only the minimum data needed for authentication."}
        />

        <Section
          delay={225}
          title="AI processing"
          body={"Kinship's AI suggestions — warm conversation openers, detecting promises you've jotted down, and end-of-season reflections — are generated by Anthropic's Claude API. When these features run, the relevant details about a single person (their name, relationship type, the notes and memories you've saved about them, and recent interactions) are sent to Anthropic's API to produce that one suggestion.\n\nWe send only what's needed for the suggestion, and only for the person you're viewing. Anthropic processes this data to return a result and, under its commercial API terms, does not use it to train its models. Your data is never sold or used for advertising.\n\nThis is on by default. You can turn it off any time in Settings → Privacy & Data → AI Personalization. When it's off, nothing is sent to Anthropic and the app uses its built-in, on-device suggestions instead. All other features work exactly the same."}
        />
